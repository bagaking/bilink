...
- 质量门禁：`make lint`、`make test`、`sh "$BAGAKIT_LIVING_KNOWLEDGE_SKILL_DIR/scripts/bagakit-living-knowledge.sh" index --root . && git diff --exit-code docs/must-guidebook.md docs/must-sop.md`。
